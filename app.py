...
def load_data():
    ...
    for name, tk in tickers.items():
        d = yf.download(tk, start='2000-01-01', end='2026-04-12', progress=False, auto_adjust=True)
...
def build_features(data):
    ...
    df['variance_trend']   = df['vol_20d'].rolling(60).apply(variance_slope, raw=False)
...
@st.cache_data
def train_model(df, split_date, drawdown_threshold, forward_window):
    future_min = df['SET_close'].rolling(forward_window).min().shift(-forward_window)
    future_drawdown = (future_min / df['SET_close']) - 1
    df = df.copy()
    df['target'] = (future_drawdown < drawdown_threshold).astype(int)
    df = df.dropna()
    ...
    train_auc = roc_auc_score(y_train, train_pred) if y_train.sum() > 0 else float('nan')
    test_auc  = roc_auc_score(y_test,  test_pred)  if y_test.sum() > 0 else float('nan')

    df['risk_score'] = np.concatenate([train_pred, test_pred]) * 100
    importances = pd.Series(model.feature_importances_, index=feature_cols).sort_values(ascending=False)

    return df, train_auc, test_auc, importances, feature_cols
...
with st.spinner("Loading market data and training model..."):
    ...
    df, train_auc, test_auc, importances, feature_cols = train_model(
        df_features, pd.Timestamp(split_date), drawdown_threshold, forward_window
    )
...
if current_score < 25:
    ...
    risk_label, risk_color = "🟠 ELEVATED", "darkorange"
else:
...
with tab1:
    ...
    axes[0].axvline(pd.Timestamp(split_date), color='black', linestyle='--', alpha=0.5, label='Train/Test split')
    for date, label in crises.items():
        d = pd.Timestamp(date)
    ...
    axes[1].axvline(pd.Timestamp(split_date), color='black', linestyle='--', alpha=0.5)
