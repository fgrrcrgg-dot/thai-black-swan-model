...
# ============================================================
# DARK THEME FOR MATPLOTLIB (matches Bloomberg terminal theme)
# ============================================================
BG_COLOR        = '#0E1117'   # matches Streamlit background
PANEL_COLOR     = '#1C1F26'   # matches secondaryBackgroundColor
TEXT_COLOR      = '#FAFAFA'
GRID_COLOR      = '#2A2E39'
ACCENT_AMBER    = '#FFA500'
ACCENT_RED      = '#DC143C'
...
with tab1:
    ...
    axes[0].plot(df.index, df['SET_close'], color=ACCENT_AMBER, linewidth=1.2)
    axes[0].axvline(split_ts, color=TEXT_COLOR, linestyle='--', alpha=0.5, label='Train/Test split')
    for date_str, label in crises.items():
        d = pd.Timestamp(date_str)
        if d >= df.index.min() and d <= df.index.max():
            axes[0].axvline(d, color=CRISIS_LABEL, alpha=0.5, linestyle=':')
            axes[0].text(d, df['SET_close'].max()*0.98, label,
                         rotation=90, fontsize=8, va='top', ha='right', color=CRISIS_LABEL)
    axes[0].set_title('SET Index with Historical Black Swan Events', fontsize=12, color=TEXT_COLOR)
    axes[0].set_ylabel('SET Index', color=TEXT_COLOR)
    axes[0].legend(loc='upper left')
    axes[0].grid(alpha=0.3, color=GRID_COLOR)

    axes[1].fill_between(df.index, 0, df['risk_score'], color=ACCENT_RED, alpha=0.7)
    axes[1].axhline(50, color=ACCENT_AMBER, linestyle='--', alpha=0.7, label='Elevated (50)')
    axes[1].axhline(75, color=ACCENT_RED, linestyle='--', alpha=0.9, label='High alert (75)')
    axes[1].axvline(split_ts, color=TEXT_COLOR, linestyle='--', alpha=0.5)
    axes[1].set_title('Black Swan Risk Index (model output, 0–100)', fontsize=12, color=TEXT_COLOR)
    axes[1].set_ylabel('Risk Score', color=TEXT_COLOR)
    axes[1].set_ylim(0, 100)
    axes[1].legend(loc='upper left')
    axes[1].grid(alpha=0.3, color=GRID_COLOR)
...
with tab2:
    ...
    importances.sort_values().plot(kind='barh', color=ACCENT_AMBER, ax=ax2)
    ax2.set_title('Feature Importance — What Predicts Thai Market Fragility', fontsize=12, color=TEXT_COLOR)
    ax2.set_xlabel('XGBoost Importance', color=TEXT_COLOR)
    ax2.grid(alpha=0.3, axis='x', color=GRID_COLOR)
    plt.tight_layout()
    st.pyplot(fig2)
